//
//  D3Constants.m
//
//
//  Created by Chen XueFeng on 16/5/19.
//
//

#import "D3Constants.h"

#pragma mark - app id
NSString *const kAppReviewURL           = @"itms-apps://itunes.apple.com/app/id1125770301";
NSString *const kAppURL                 = @"http://itunes.apple.com/app/id1125770301";


#pragma mark - privacy url
NSString *const kPrivacyURL             = @"http://www.jianshu.com/p/1a87dcdfb795";

#pragma mark - IAP
NSString *const kIAPAdRemoved           = @"com.inspirelife.d3storm.adremoved";
NSString *const kIAPVip                 = @"com.inspirelife.d3storm.vip";

#pragma mark - Leancould
NSString *const kLeanCloudApplicationId = @"KSJlHLkP8xfDt8uJbI9gzMGf-gzGzoHsz";
NSString *const kLeanClientKey          = @"AgoYmi4XOYqw4Sr2VRaVN1FL";

#pragma mark - Share SDK
NSString *const kShareSDKApplicationId  = @"143407f1ccebc";

#pragma mark - WeiXin
NSString *const kWXApplicationId        = @"wx414d8ef2b1cd0dd6";
NSString *const kWXApplicationSecret    = @"ddf1148d1c8066765dba412bd11e22c5";

#pragma mark - QQ
NSString *const kQQApplicationId        = @"1105418831";
NSString *const kQQApplicationSecret    = @"hGAsrLT052br3nAq";

#pragma mark - Userdefault
NSString *const kUserDefaultFirstLaunch   = @"com.inspirelife.first.launch";
NSString *const kUserDefaultIsVip         = @"com.inspirelife.d3storm.vip";
NSString *const kUserDefaultIsAdRemoved   = @"com.inspirelife.d3storm.adremoved";
NSString *const kUserDefaultIsSharedToday = @"com.inspirelife.shared.today";
NSString *const kUserDefaultCoin          = @"com.inspirelife.coin";
NSString *const kUserDefaultLastLoginDate = @"com.inspirelife.last.login.date";
NSString *const kUserDefaultVersion12     = @"com.inspirelife.version.12";

NSString *const kUserDefaultDailyNotificationStatus = @"com.inspirelife.daily.notification.status";
NSString *const kUserDefaultDailyNotificationTime   = @"com.inspirelife.daily.notification.time";

#pragma mark - Notification
NSString *const kNotificationVIPChanged  = @"notification.vip.changed";
NSString *const kNotificationCoinChanged = @"notification.coin.changed";

#pragma mark - Admob
NSString *const kAdmobBannerID          = @"ca-app-pub-2906017339602978/9297974646";
NSString *const kAdmobInterstitialId    = @"ca-app-pub-2906017339602978/1635107040";
