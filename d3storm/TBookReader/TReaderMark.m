...
{
    NSString *contion = [NSString stringWithFormat:@"bookId = '%ld'",(long)(long)bookId];
...
{
    NSString *contion = [NSString stringWithFormat:@"bookId = '%ld' and chapterIndex = '%@'",(long)(long)bookId,chapterName];
    return [self dbObjectsWhere:contion orderby:nil];
}
...
- (BOOL)removeDbObjectsWhereRange:(NSRange)range
{
    ...
    for (TReaderMark *mark in array) {
        if (NSLocationInRange(mark.offset, range)) {
            NSString *contion = [NSString stringWithFormat:@"bookId = '%ld' and chapterIndex = '%@' and offset = '%ld'",(long)_bookId,_chapterIndex,(long)mark.offset];
