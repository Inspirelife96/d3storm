...
- (void)viewDidLoad {
    ...
    NSString *bookIdString = [NSString stringWithFormat:@"%ld", (long)_readerBook.bookId];
    NSDictionary *dic = [userDefault objectForKey:bookIdString];
    if (dic == nil) {
        [self openBookWithChapterIndex:0];
        [self showReaderPage:0];
    } else {
        NSInteger chapter = [[dic objectForKey:@"chapter"] integerValue];
        NSInteger offset = [[dic objectForKey:@"offset"] integerValue];
        [self turnToBookChapter:chapter chapterOffset:offset];
    }
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(chapterChanged:) name:TReaderChapterChangeNofication object:nil];
}

- (void)viewWillDisappear:(BOOL)animated {
    [[NSNotificationCenter defaultCenter] removeObserver:TReaderChapterChangeNofication];
}

-(void)chapterChanged:(NSNotification*) notification {
    ...
    NSNumber *chapter = [userInfo objectForKey:@"chapter"];
    
    [self turnToBookChapter:[chapter integerValue]];
}
...
- (void)addPageViewController
{
    ...
    CGFloat adHeight = 8.0f;
    
    if (!IsAdRemoved) {
        if (self.view.frame.size.height > 720.f) {
            adHeight = 90.0f;
        } else if (self.view.frame.size.height > 400.f) {
            adHeight = 50.0f;
        } else {
            adHeight = 30.0f;
        }
    ...
    pageViewController.view.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - adHeight);
...
//        _readerBook.bookName = @"yhzj";
//        _readerBook.totalChapter = 24;
//    }
...
- (void)showPageIndexViewWithPage:(NSInteger)page totalPage:(NSInteger)totalPage
{
    ...
    _pageIndexView.label.text = [NSString stringWithFormat:@"%ld/%ld",(long)page,(long)totalPage];
}
      ...
      viewControllerBeforeViewController:(UIViewController *)viewController
{
    ...
    NSLog(@"总页码%ld 当前页码%ld",(long)chapter.totalPage,_curPage+1);
...
//        [self.navigationController pushViewController:VC animated:YES];
        TReaderChapterListViewController *chapterListVC = [[TReaderChapterListViewController alloc] initWithStyle:UITableViewStyleGrouped];
        chapterListVC.chapterList = _readerBook.chapterList;
...
- (void)readerTopBar:(EReaderTopBar *)readerTopBar didClickedAction:(EReaderTopBarAction)action
{
    if (action == EReaderTopBarActionBack) {
        ...
        TReaderPager *pager = [_chapter chapterPagerWithIndex:_curPage];
        
        
        NSNumber *chapterNumber = [NSNumber numberWithInteger:_chapter.chapterIndex];
        NSNumber *offset = [NSNumber numberWithInteger:pager.pageRange.location];
        
        NSDictionary *dic = @{@"chapter":chapterNumber, @"offset":offset};
        
        NSString *bookIdString = [NSString stringWithFormat:@"%ld", (long)_readerBook.bookId];
        [userDefault setObject:dic forKey:bookIdString];
